...
export function TeacherLayout({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)] xl:gap-5" data-testid="teacher-layout">
      {/* Desktop sidebar */}
      <aside className="hidden lg:block" data-testid="teacher-sidebar">
        ...
        >
          <div className="mb-4 px-2 text-h3 text-text-title">Teacher</div>
          <SidebarNav groups={groups as any} />
          <div className="mt-6 border-t border-border-subtle pt-3">
            <Button className="w-full" variant="secondary" tone="neutral" onClick={logout} data-testid="teacher-sidebar-logout">
              Logout
            </Button>
          </div>
        </div>
                ...
                >
                  <Dialog.Panel className="paper-primary max-h-[calc(100vh-2rem)] w-full max-w-sm overflow-y-auto overscroll-contain p-3 shadow-lg">
                    <div className="mb-3 flex items-center justify-between gap-3 px-2">
                      <Dialog.Title className="text-sm font-semibold text-text-title">Teacher menu</Dialog.Title>
