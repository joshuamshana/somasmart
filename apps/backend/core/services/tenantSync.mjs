import { blobNeedSchema, syncPullSchema, syncPushSchema } from "../contracts.mjs";
import { requireTenantAccess } from "./helpers.mjs";
function canReadChangeForRole(change, userId, role) {
    if (role === "admin" || role === "school_admin")
        return true;
    if (change.entityType !== "users")
        return true;
    return change.entityId === userId;
}
export async function registerTenantSyncRoutes(app) {
    ...
    app.post("/sync/pull", async (request, reply) => {
        ...
        const since = typeof explicitCursor === "number"
            ? explicitCursor
    ...
    app.get("/sync/blob/:cid", async (request, reply) => {
        ...
        if (!params.cid)
            return reply.status(400).send({ code: "CID_REQUIRED" });
        const manifest = await app.store.getBlobManifest(claims.projectId, params.cid);
        if (!manifest)
            return reply.status(404).send({ code: "BLOB_NOT_FOUND" });
