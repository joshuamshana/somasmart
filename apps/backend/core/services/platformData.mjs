import { platformDataExportSchema, platformDataMutationsSchema, platformDataReindexSchema } from "../contracts.mjs";
import { getTraceId, requirePlatformAccess } from "./helpers.mjs";
export async function registerPlatformDataRoutes(app) {
    ...
    app.get("/platform/jobs/:jobId", async (request, reply) => {
        ...
        const params = request.params;
        if (!params.jobId)
            return reply.status(400).send({ code: "JOB_ID_REQUIRED" });
        const job = await app.store.findPlatformJobById(params.jobId);
        if (!job)
            return reply.status(404).send({ code: "JOB_NOT_FOUND" });
